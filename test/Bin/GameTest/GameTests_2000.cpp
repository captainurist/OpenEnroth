...
GAME_TEST(Prs, Pr2354) {
    ...
    for (MapId mapId : allMaps()) {
        ...
        if (isMapIndoor(mapId)) {
            ...
            deserialize(lod::decodeMaybeCompressed(pGames_LOD->read(fileName).orThrow()).orThrow(), &rawLocation).orThrow();
            ...
            deserialize(lod::decodeMaybeCompressed(pGames_LOD->read(dlvFilename).orThrow()).orThrow(), &rawDelta, tags::context(rawLocation)).orThrow();
            ...
            deserialize(lod::decodeMaybeCompressed(pGames_LOD->read(ddmFilename).orThrow()).orThrow(), &rawDelta, tags::context(rawLocation)).orThrow();
            reconstruct(rawDelta, &location);
        }
    }
}
