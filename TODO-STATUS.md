...
Every `TODO` in `src/` and `test/` re-verified against master `3a07d7bc4e2` (Turn CullFaceToCameraFrustum into a pure predicate (#2627)) on 2026-08-31.

This is a refresh of `todos.md` on the `todos` branch, which was last written against `23f7cce94ce`. Every open entry was
grepped against the current tree, entries whose TODO text is gone are ticked, entries whose file moved carry the new path,
and the line numbers below are current rather than the drifted ones in `todos.md`.

The fix sketches under each entry are carried over verbatim from `todos.md` and were written by the earlier
triage pass, so their line references are the old ones and their prose is not restyled.

Local working state. Not to be committed.

## What changed since the last refresh

- **13 entries resolved on master** and are ticked below.
- **12 entries moved files** - the Trace and Time libraries went into `src/Core`, `SoundEnums.h` into `src/Engine/Data`.
- **2 new TODOs** appeared and are triaged here for the first time (`EvtInterpreter.cpp` MM8 mapping, `Decoration.h` global).
- **1 entry became stale by being fixed around it**: `Core/Trace/EventTrace.h` still says "this should go to Core/" after #2582 moved it there.

Resolved since `23f7cce94ce`:

- `src/Engine/Graphics/Camera.cpp:308` (Trivial) - TODO(yoctozepto): just have this as a global constant instead of random vars/4 around
- `src/Engine/Engine.cpp:446` (Easy) - TODO(captainurist): Right now we can have popups for monsters that are not reachable with a bow, and
- `src/Engine/Evt/EvtInstruction.cpp:874` (Easy) - TODO(yoctozepto): this downcasts a DWORD to WORD
- `src/Engine/Evt/EvtInstruction.cpp:1042` (Easy) - TODO(yoctozepto): this downcasts a DWORD to WORD
- `src/Engine/Evt/EvtInstruction.cpp:1162` (Easy) - TODO(yoctozepto): this downcasts a DWORD to WORD
- `src/Engine/Graphics/Camera.cpp:217` (Easy) - TODO(pskelton): does this func need to copy verts or could it be eliminated
- `src/Engine/Graphics/Collisions.cpp:465` (Easy) - TODO: this doesn't respect ignore_ethereal parameter
- `src/Engine/Graphics/Indoor.cpp:205` (Easy) - TODO(captainurist): using pEventTimer here is weird. This means that e.g. cleric in the haunted mans
- `src/Engine/Spells/CastSpellInfo.cpp:2204` (Easy) - TODO(captainurist): investigate, that's a very weird std::to_underlying call.
- `src/Engine/Graphics/BSPModel.h:12` (Medium) - visibility flag TODO(pskelton): use for map tooltip checking or remove
- `src/Engine/Graphics/Renderer/OpenGLRenderer.cpp:2687` (Medium) - TODO(pskelton): clean up / need to stack decals
- `src/Io/InputEnumFunctions.h:25` (Medium) - find a better place for this code
- `src/Library/Trace/EventTrace.cpp:12` (Medium) - #include "Io/InputEnumFunctions.h" // doesn't belong here

## Counts

| tier | open | ticked | total |
|---|---:|---:|---:|
| Trivial | 12 | 28 | 40 |
| Easy | 136 | 32 | 168 |
| Stale | 7 | 0 | 7 |
...
Ranked by value over effort. Every one was grepped against `3a07d7bc4e2`, so the line numbers are current
and the TODO is confirmed to still exist. Collisions with open PRs are called out.

### 1. Sweep the Stale tier - 7 comment deletions, one PR  — **shipped as #2688, awaiting review**

The whole tier is comments that describe work already done. No code changes, no behavior risk, and it retires
a tier outright.

| where | why it is stale |
|---|---|
| `src/Engine/Evt/EvtInstruction.cpp:887` | The zero-init it asks for is on the next line already (`EvtInstruction ir = {}`) |
| `src/Engine/Graphics/Collisions.cpp:922` | pskelton's reply on the following line answers the question |
| `src/Engine/Graphics/Renderer/OpenGLRenderer.cpp:760` | #2309 replaced the fixed store with a growing vector, there is no "full" state to force on |
| `src/Engine/Graphics/Renderer/OpenGLRenderer.cpp:2750` | `glbspshader.frag` now derives the size via `textureSize()`, nothing needs passing through. The identical copy at `:2250` is filed under Design as a genuine open question about the outbuild shader, so leave that one alone |
| `src/Engine/Graphics/Renderer/OpenGLRenderer.cpp:3029` and `:3044` | Artifacts of the old `pVertexUIDs`/`pTextureUIDs` names, the fields are plain `textureUs`/`textureVs` now |
| `src/Engine/Graphics/Renderer/OpenGLRenderer.cpp:3513` | The two lines below already call `assets->releaseAllTextures()` plus ReleaseTerrain/ReleaseBSP |

**Plus one this refresh found:** `src/Core/Trace/EventTrace.h:14` still reads
`TODO(captainurist): this should go to Core/, not Library/,` - and #2582 moved it into `src/Core` on 2026-08-07. It is
filed under Design in the old list because the move was a real question then. It is a one-line deletion now.

### 2. `src/Engine/mm7_data.cpp:8` - move the sphere verts to where they are used  — **shipped as #2696, awaiting review**

`TODO(pskelton): move me - to spellfxrendere.h?`

`sphereVertPos` and `sphereVertInd` are declared in `SpellFxRenderer.h:19-20` and used only by
`SpellFxRenderer.cpp:119-128`, but defined in the `mm7_data.cpp` junk drawer. Move the two definitions across.
Two files, no behavior change.

### 3. `src/GUI/GUIWindow.h:86` - delete a parameter nobody passes  — **shipped as #2693, awaiting review**

`TODO(pskelton): inside_game_viewport used anywhere?`

The answer is no. All 19 call sites pass `0` or `false` - 15 of them in `UIPopup.cpp`, the rest in
`UIGameOver.cpp`, `UIMessageScroll.cpp`, `UIPartyCreation.cpp` and `LoadStep2State.cpp` - so the parameter and the
dead `if (inside_game_viewport)` branch at `GUIWindow.cpp:212` both go. A compiler-checked blast radius.

### 4. `src/Engine/Graphics/LocationInfo.h:4` - the header is not graphics  — **shipped as #2706, awaiting review**

`TODO(captainurist): this isn't graphics, it belongs in Engine/ rather than Engine/Graphics/.`

`git mv` to `src/Engine/`, fix the CMakeLists entry and four includes (`Indoor.h:19`, `LocationFunctions.h:5`,
`Outdoor.h:15`, `Temple.cpp:13`). Note line 3 of the same file is a second, larger TODO asking to rename the type
with a `Map-` prefix - decide up front whether to do just the move or fold both together.

### 5. `src/Engine/Resources/LodTextureCache.h:27-28` - two pointless forwarders  — **NOT easy, re-triaged 2026-09-03**

Both say `doesn't belong here.` `LoadCompressedTexture` forwards to `lod::decodeMaybeCompressed(...)`, `read()`
forwards to `_reader.read()`. The obvious fix is a `reader()` accessor, but this is a design change and not a
cleanup. Three things the earlier note got wrong or missed:

- The user counts were taken from a grep that conflated two classes. Most `->read(` sites in the tree are on
  `pGames_LOD`, which is a `LodReader` and not a `LodTextureCache`. The real users are `GUIFont.cpp:36` and
  `ImageLoader.h:117` for `LoadCompressedTexture`, and `TileGenerator.cpp:93` alone for `read()`.
- `ImageLoader.h` forward declares `LodTextureCache` and `LodReader` rather than including them, which is why
  `PCX_LOD_Compressed_Loader` is a template at all. Dropping the forwarder pushes an include of
  `Library/LodFormats/LodFormats.h` into that header, so it is a coupling change.
- The replacement expression would be `lod::decodeMaybeCompressed(lod->reader().read(...))`, and inside that
  lambda the namespace `lod` is shadowed by the parameter `lod`. It needs `::lod::` or a renamed parameter.

Two side findings from the same read. `PCX_LOD_Compressed_Loader` is instantiated exactly once, always with a
`LodTextureCache *`, so the template buys nothing but the forward declaration. And `PCX_LOD_Raw_Loader` has no
instantiation anywhere in src or test, so it looks like dead code.

### 6. `src/Io/Mouse.cpp:253` - a real bug, not just a cleanup  — **#2721 then #2723, #2717 closed**

`actual shape is oval, this check is bugged.`

The click test uses `sqrt(dx*dx+dy*dy) < rect.w`, a circle of radius = width, while the hover test at
`UIGame.cpp:1041-1051` uses the correct ellipse test. This one changes behavior, so it needs a game test and a
retrace check rather than a build-and-push - which is exactly why it is worth doing deliberately rather than
picking it up by accident.

Outcome, split in two on the user's call. The real defect was that `rect` held the oval's center in `x`/`y` and its
semi-axes in `w`/`h` for `BUTTON_TYPE_CHARACTER`, so #2721 makes `rect` the bounding box with no behavior change,
and #2723 then fixes the click test on top of it. The click test used the inscribed circle, so a nine pixel band at
the top and the bottom of each of the four party portraits showed the character in the status bar on hover but did
not click. The two hover
tests in `UIGame.cpp` already had the ellipse right and were verbatim copies, so all three now call
`GUIButton::containsOval`. The `UISpell.cpp` targeting portraits are round, which is why nobody noticed. No retrace
was needed, the change adds no RNG call and all 364 existing game tests stayed green.

### 7. Mechanical renames - pick one, they are all zero-risk and compiler-verified

| where | scope |
|---|---|
| `src/Engine/stru314.h:5` | Rename the struct and its `field_*` members to a decal basis - users are `stru314.cpp`, `DecalBuilder.cpp/h` and two CMakeLists lines |
| `src/Engine/Objects/NPCEnums.h:26` | `NpcProfession`'s 59 CamelCase enumerators to type-prefixed SNAKE_CASE - the `NPC_PROFESSION_FIRST`/`_LAST` aliases already follow the rule |
| `src/Io/KeyboardActionMapping.h:18` | `TextInputType` None/Text/Number to `TEXT_INPUT_TYPE_*`, about 15 sites |
| `src/Engine/Spells/SpellBuff.h:7` | Method names to lowercase-initial per HACKING.md, plus the Hungarian params |
| `src/Engine/Pid.h:7` | `OBJECT_None` to `OBJECT_NONE` and friends - **402 occurrences**, correct but a huge diff to review |
| `src/Utility/System/NativePath.h:37` | `fromWtf8`/`toWtf8` are misnomers, 46 references in 12 files, but the new name needs picking first |

### New easy candidates found while doing the picks above

- [x] **`src/Engine/CMakeLists.txt` - four headers are not style checked.** SHIPPED as #2710, which widened it
  to all eleven unlisted headers across six CMakeLists. A rescan after that PR finds zero unreferenced headers
  in `src` and `test`, and zero stale entries, so this axis is closed.
- [x] **`src/Library/Binary/PackSerialization.h` is dead and does not compile.** SHIPPED as the second commit on
  #2710, which deletes the file. It was added by #2374 already referencing `TagPack`, a template defined nowhere
  in the tree at that commit or any other, so it never compiled. Invisible because CMake does not compile headers
  and cpplint does not parse C++.
- **`src/Engine/MapEnumFunctions.h` - four prose-only doxygen blocks on functions.** Three only restate the
  function name, and line 27 reads `Is hirelings interactions are forbidden on this map?`. CLAUDE.md wants a
  `@param` per parameter and a `@return`, so either finish them or delete the three that add nothing.
- **`src/Engine/HitMap.h` includes `<algorithm>` and uses nothing from it.** Wants its own commit, since dropping
  an include from a header included by `Renderer.h` and `UICharacter.h` can break a TU that leaned on it
  transitively. Also `HitMap::Entry`'s constructor takes `T val` by value and copies it into the member, which
  defeats the `std::move` in `add()`.
- **Platform-gated style targets never run in CI.** `check_style_library_environment_android`,
  `check_style_library_environment_win` and `check_style_main` sit inside platform `if` blocks, and `lint.yml`
  runs `check_cpp_style` only on ubuntu. Running cpplint on the Android pair by hand reports 4 findings.
- **`src/Engine/mm7_data.cpp` - five globals with no reader.** `uGammaPos` and `_4E94D2_light_type` are still
  declared in `mm7_data.h`, while `_4D864C_force_sw_render_rules`, `byte_4FAA24` and `byte_5E4C15` are not
  declared at all. All five have a definition and zero uses in `src` or `test`. A deletion, so it wants its own PR.
- **`src/Engine/Graphics/ImageLoader.h` - `PCX_LOD_Raw_Loader` has no instantiation** anywhere in `src` or
  `test`, only the class and its `Load` definition. Looks like dead code, needs one careful check that nothing
  constructs it through a factory before deleting.

### Watch out for these

- **`src/GUI/UI/UIHouses.cpp:691`** (`TODO(captainurist): encapsulate`) sits in the same file as open PR #2668.
  Wait for that to land or you will be rebasing.
- **`src/Engine/Objects/ActorEnums.h:74`** (rename `AIState::Summoned` to `Summoning`) touches `Actor.cpp`
  heavily - 12 of its ~25 use sites are there - and `Actor.cpp` is what open PR #2614 changes. The enum is used
  unqualified through `using enum`, so grep for the bare word and skip `LSTR_SUMMONED` and the prose comment at
  `Actor.cpp:178`.
- **`src/GUI/UI/UIHouses.cpp:669` and `:672`** are already being removed by #2668 - do not start them.
- Anything that changes game logic or adds an RNG call needs a retrace, per CLAUDE.md.


## Trivial (12 open of 40)
...
- [x] **308** - TODO(yoctozepto): just have this as a global constant instead of random vars/4 around
...
- [x] **4** - TODO(captainurist): this isn't graphics, it belongs in Engine/ rather than Engine/Graphics/.
  - Done in PR #2706. The header is now src/Engine/LocationInfo.h, both CMakeLists updated, four includes fixed. The three siblings had it in their local relative include group, so it was lifted into their Engine/ group rather than left there. The line-3 Map- rename was deliberately left alone as a separate naming question.
...
- [x] **8** - TODO(pskelton): move me - to spellfxrendere.h?
  - Done in PR #2696. Moved into SpellFxRenderer.cpp as static constexpr, and the extern declarations were deleted rather than moved, because a header is not where a 133-line table belongs and six TUs include that header without touching the data.

**Follow-ups found while doing #2696, each its own change:**

- Five more constant tables in mm7_data.cpp have the same single-reader shape: `flt_4D84E8` (PortalFunctions.cpp), `flt_4E4A80` (Actor.cpp), `byte_4ECF08` (Character.cpp), `speechVariants` (Character.cpp) and `base_recovery_times_per_weapon_type` (Character.cpp). Each lands in a different file, so they are five reviews and not one.
- Five globals there look dead outright, defined with no reader anywhere in src or test: `uGammaPos` and `_4E94D2_light_type` (both still declared in mm7_data.h), plus `_4D864C_force_sw_render_rules`, `byte_4FAA24` and `byte_5E4C15` which are not declared at all. Verified by grep over src and test on master 7314491173c. That is a deletion, so it wants its own PR.
...
- [ ] **669** - TODO(Nik-RE-dev): looks like this function is not needed anymore **[in #2668]**
...
- [ ] **672** - TODO(Nik-RE-dev): Looks like it's artifact of MM6 **[in #2668]**
...
- [ ] **691** - TODO(captainurist): encapsulate
...
- [ ] **98** - TODO(captainurist): Move to [audio]?
...
- [ ] **590** - TODO: deadzone should be configurable and default should be lowered once we implement proper axis event processing.
...
- [ ] **2846** - TODO(pskelton): stop audio comment?
...
### `src/Core/Time/Duration.h`

- [ ] **95** - TODO(captainurist): #time add unit tests. _(was `src/Engine/Time/Duration.h:95`)_
...
- [ ] **306** - TODO(pskelton): set this on level load
...
- [x] **446** - TODO(captainurist): Right now we can have popups for monsters that are not reachable with a bow, and this is OK. However, such monsters also don't get a hint displayed on mouseover. Probably should fix this?
...
- [x] **874** - TODO(yoctozepto): this downcasts a DWORD to WORD
...
- [x] **1042** - TODO(yoctozepto): this downcasts a DWORD to WORD
...
- [x] **1162** - TODO(yoctozepto): this downcasts a DWORD to WORD
...
- [ ] **119** - TODO(pskelton): fov calcs only need recalculating on level change or if we add config option
...
- [x] **217** - TODO(pskelton): does this func need to copy verts or could it be eliminated
...
- [x] **465** - TODO: this doesn't respect ignore_ethereal parameter
...
- [x] **205** - TODO(captainurist): using pEventTimer here is weird. This means that e.g. cleric in the haunted mansion is not animated in turn-based mode. Use misc timer?
...
- [ ] **422** - TODO(captainurist): asserts are on in release builds, so the line below is unreachable. Work out whether every face can really be above the party here, and drop it if not.
...
- [ ] **1005** - TODO(captainurist): can drop this? Was loaded because light elementals can be summoned.
...
- [ ] **1137** - TODO(pskelton): common emit fire code
...
- [ ] **1266** - TODO(captainurist): what's going on in this check?
...
- [ ] **1338** - TODO(Nik-RE-dev): does not belong here, it's common function for interaction for both indoor/outdoor
...
- [ ] **1339** - TODO(Nik-RE-dev): get rid of external function declaration inside
...
- [ ] **1847** - TODO(pskelton): configurable thresholds for treasure
...
- [ ] **267** - TODO(pskelton): move to party?
...
- [ ] **19** - TODO(pskelton): this needs reworking if we want lights to be outlined _(was `src/Engine/Graphics/LightmapBuilder.cpp:21`)_
...
- [ ] **7** - TODO(captainurist): move to Engine/ and drop the Location- prefix, should be MapSmth.
...
- [ ] **773** - TODO(pskelton): what is this for?
...
- [ ] **913** - TODO(pskelton): Pass party as param
...
- [ ] **182** - TODO(Nik-RE-dev): check colour format use in particles
...
- [ ] **225** - TODO(pskelton): reinstate this guard check
...
- [ ] **142** - TODO(pskelton): Move to outdoors - clean up
...
- [ ] **236** - TODO(pskelton): what is this for?
...
- [ ] **306** - TODO(captainurist): what's going on here?
...
- [ ] **364** - TODO(pskelton): replace asserts with warning and return false
...
- [ ] **410** - TODO(captainurist): taking cos of an INT angle? WTF?
...
- [ ] **526** - TODO(captainurist): Old code used BR - TL (one less than actual width/height), preserving that behavior for now.
...
- [ ] **537** - TODO: should this be combined / moved out of render
...
- [ ] **130** - TODO(pskelton): clean up and move out of here
...
- [ ] **785** - TODO(captainurist): subImage().scale()
...
- [ ] **1147** - TODO(captainurist): terrible, terrible hack, redo this.
...
- [ ] **1229** - TODO(pskelton): this should be a separate function
...
- [ ] **1415** - TODO(pskelton): do we need this?
...
- [ ] **3361** - TODO(captainurist): this is probably the place to check OpenGL version & exit. openenroth requires opengl core 4.1 or opengles 3.2 capable gpu to run.
...
- [ ] **41** - TODO(captainurist): what if fmt throws?
...
- [ ] **541** - TODO(captainurist): this doesn't belong to a getter!!!
...
- [ ] **3042** - TODO(Nik-RE-dev): this looks like some artifact from MM6 (where you can eat reagents) In MM7 these item IDs are invalid (plus index 161 used twice which is wrong)
...
- [ ] **3217** - TODO(Nik-RE-dev): why not playerAffected?
...
- [ ] **3624** - TODO(captainurist): values coming from scripts should be bound-checked.
...
- [ ] **4078** - TODO(captainurist): qbits value is coming from a script, need to bound-check.
...
- [ ] **4691** - TODO(captainurist): quest bit is coming from a script, do range checking here.
...
- [ ] **5205** - TODO(captainurist): quest bit is coming from a script, do range checking here.
...
- [ ] **5606** - TODO(Nik-RE-dev): decreasing 1 seems wrong, also bits indexing was changed
...
- [ ] **5952** - TODO(captainurist): doesn't look like a proper default.
...
- [ ] **6535** - TODO(captainurist): encapsulate the logic here.
...
- [ ] **6615** - TODO(captainurist): just implement properly?
...
- [ ] **6622** - TODO(_): probably move this somewhere else, not really Character:: stuff
...
- [ ] **131** - TODO(Nik-RE-dev): chest is originator in this case
...
- [ ] **161** - TODO(captainurist): need to use mouse->pickedItemOffset here?
...
- [ ] **335** - TODO(captainurist): GenerateArtifact calls Reset on failure, this messes up inventory state. Rewrite properly.
...
- [ ] **158** - TODO(captainurist): can drop potionPower?
...
- [ ] **439** - TODO(captainurist): makes no sense.
...
- [ ] **40** - TODO(captainurist): that's... a weird default.
...
- [ ] **46** - TODO(captainurist): and here, weird default.
...
- [ ] **97** - TODO(captainurist): This looks broken. pNPCTopics[407].pTopic = "Mind Guild Membership" pNPCTopics[407].pText = "With Expert Air Magic you can learn all of the Expert spells for this element...." Double broken!
...
- [ ] **238** - TODO: why pActors.size() - 1? Should just check for .size()
...
- [ ] **381** - TODO(Nik-RE-dev): check purpose of inner loop
...
- [ ] **8** - dependency doesn't belong here
...
- [ ] **71** - TODO (mcgreentn): should we use an epsilon here?
...
- [ ] **505** - TODO(Nik-RE-dev): check colour format
...
- [ ] **566** - TODO(captainurist): why transparent black?
...
- [ ] **609** - TODO(captainurist): why transparent black?
...
- [ ] **1662** - TODO(captainurist): why transparent black?
...
- [x] **2204** - TODO(captainurist): investigate, that's a very weird std::to_underlying call.
...
- [ ] **2638** - TODO: why call SetCondition and then conditions.set?
...
- [ ] **91** - TODO(captainurist): #enum use enum serialization
...
- [ ] **156** - TODO(captainurist): Sus. Comparing height to width?
...
- [ ] **184** - TODO(captainurist): logger can be NULL here if we're called from cxa_finalize.
...
- [ ] **943** - TODO(captainurist): encapsulate this logic in PriceCalculator
...
- [ ] **49** - TODO(pskelton): move to button
...
- [ ] **85** - TODO(pskelton): string_view or ref?
...
- [x] **86** - TODO(pskelton): inside_game_viewport used anywhere?
  - Done in PR #2693. No: all 19 call sites (15 in UIPopup.cpp, one each in UIGameOver, UIPartyCreation, UIMessageScroll, LoadStep2State) pass 0/false, so the parameter and the dead pViewport branch both went. The earlier note said 21, which was the grep hit count including the declaration and the definition.

### `src/GUI/UI/Books/TownPortalBook.cpp`

- [ ] **211** - render->DrawQuad2D(game_ui_statusbar, {0, 352}); // TODO(captainurist): engine->_statusBar->smthSmth()???
...
- [ ] **43** - TODO(captainurist): #time gotta be dt-based.
...
- [ ] **358** - TODO(captainurist): #unicode this is not ascii
...
- [ ] **360** - TODO(captainurist): #unicode this is not ascii
  - The hireling-slot-1 half of the same comparison; fixed by the same one-line change to `==`.

### `src/GUI/UI/UIGame.cpp`

- [ ] **252** - TODO(pskelton): #time check tickcount usage here
...
- [ ] **976** - TODO(captainurist): get rid of this std::to_underlying cast.
...
- [ ] **1353** - TODO(pskelton): actually check for party movement
...
- [ ] **752** - TODO(pskelton): why just CHARACTER_BUFF_RESIST_WATER?
...
- [ ] **253** - actual shape is oval, this check is bugged.
...
- [ ] **127** - this is not symmetric with that's done in read / openForReading / openForWriting.
...
- [ ] **887** - TODO(yoctozepto): zeroing-out the struct to prevent values from previous events from lingering; this makes it slightly easier to spot uninitialised members but, since the 0s may have a proper meaning, not always;
...
- [ ] **922** - TODO(captainurist): why there is no call to _46ED8A_collide_against_sprite_objects? See ProcessPartyCollisionsODM.
...
- [ ] **760** - TODO (pskelton): should force drawing if buffer is full
...
## Medium (183 open of 191)

_Clear goal, but touches several files or needs care and tests._

### `src/Application/Game.cpp`

- [ ] **817** - TODO(Nik-RE-dev): need separate message type
...
- [ ] **30** - TODO(captainurist): apply codestyle here.
...
- [ ] **123** - TODO(captainurist): move all Trace* options into a separate section.
...
- [ ] **468** - TODO(captainurist): #enum, will need to support cycleIncrement for enums entries.
...
- [ ] **612** - TODO(captainurist): move to [audio]?
...
- [ ] **627** - TODO(captainurist): move to [audio]?
...
### `src/Core/Time/Time.h`

- [ ] **76** - TODO(captainurist): #time doesn't belong to GameTime. _(was `src/Engine/Time/Time.h:76`)_
...
### `src/Core/Trace/EventTrace.h`

- [ ] **52** - std::string saveFileChecksum; _(was `src/Library/Trace/EventTrace.h:52`)_
...
- [ ] **105** - TODO(captainurist): this really should be just a JSON diff.
...
- [ ] **320** - TODO(captainurist): this math makes no sense
...
- [ ] **623** - TODO(captainurist): on error in `open` we had this: Error(localization->str(LSTR_MIGHT_AND_MAGIC_VII_IS_HAVING_TROUBLE), localization->str(LSTR_REINSTALL_NECESSARY)); however, at this point localization isn't initialized yet, so this was a guaranteed crash. Implement proper user-facing error reporting!
...
- [ ] **1458** - TODO(captainurist): six more sites set _pendingTransition and uGameState by hand, route them through here.
...
- [ ] **902** - TODO(captainurist): verify enum ranges here.
...
- [ ] **200** - TODO(captainurist): ir.data.sound_descr.x, ir.data.sound_descr.y used to be passed in.
...
- [ ] **92** - TODO(Nik-RE-dev): using time of last visit will help timers only slightly because each map leaving resets it. To support fair timers they need to be saved directly.
...
- [x] **12** - visibility flag TODO(pskelton): use for map tooltip checking or remove
...
- [ ] **57** - TODO(pskelton): swap components to match expectation - eg x is depth make it z
...
- [ ] **437** - TODO(captainurist): code looks very similar to stru314::computeBasis
...
- [ ] **1202** - TODO(pskelton): Need to add check against terrain
...
- [ ] **1651** - TODO(Nik-RE-dev): use calculated velocity of party and walk/run flags instead of delta
...
- [ ] **1711** - TODO(pskelton): investigate why this happens
...
- [ ] **189** - TODO(pskelton): consider order of drawing / lighting
...
- [ ] **910** - TODO(pskelton): Magic numbers
...
- [ ] **1449** - TODO(Nik-RE-dev): use calculated velocity of party and walk/run flags instead of delta
...
- [ ] **1659** - TODO(pskelton): common gravity code extract
...
- [ ] **62** - TODO(captainurist): also sets uDefaultTravelTime_ByFoot and clears party flags as a side effect, and the travel dialog calls it every frame just to draw the map name. Split the lookup from the commit.
...
- [ ] **29** - TODO(pskelton): eliminate this one
...
- [ ] **468** - TODO(pskelton): could draw in 3d rather than convert to billboard for ogl
...
- [ ] **67** - TODO(pskelton): Combine and contain
...
- [ ] **223** - TODO(Gerark) Should we bind to the framebuffer before clearing?
...
- [ ] **406** - TODO(pskelton): do these need batching?
...
- [ ] **564** - TODO(pskelton): sort this - forcing the draw is slow
...
- [ ] **648** - TODO(pskelton): repeated maths could be saved when calculating sky planes
...
- [ ] **995** - TODO(pskelton): move this to map loading
...
- [ ] **1175** - TODO: OpenGL ES doesn't provide wireframe functionality so enable it only for classic OpenGL for now
...
- [ ] **1249** - TODO(pskelton): make this configurable - also lights should be sorted by distance so nearest are used first
...
- [ ] **1267** - TODO(pskelton): make this configurable - also lights should be sorted by distance so nearest are used first
...
- [ ] **1296** - TODO: OpenGL ES doesn't provide wireframe functionality so enable it only for classic OpenGL for now
...
- [ ] **1301** - TODO(pskelton): clean up and move to seperate function in decal builder
...
- [ ] **1340** - TODO(pskelton): terrain and boxes should be saved for easier retrieval
...
- [ ] **2289** - TODO(pskelton): Same as indoors. When ODM and BLV face is combined - seperate out function
...
- [x] **2687** - TODO(pskelton): clean up / need to stack decals
...
- [ ] **2806** - TODO(pskelton): any instances where animTime is not consistent would need checking
...
- [ ] **2879** - TODO(pskelton): set to water for now - fountains in walls of mist
...
- [ ] **3044** - TODO: OpenGL ES doesn't provide wireframe functionality so enable it only for classic OpenGL for now
...
- [ ] **1655** - TODO(captainurist): I don't like this logic. We first take the weapon with larger recovery time, then apply recovery bonuses. Should be the other way around.
...
- [ ] **2156** - TODO(_) would be nice to move these into separate functions
...
- [ ] **3343** - TODO(Nik-RE-dev): spell scroll is removed before actual casting and will be consumed even if casting is canceled.
...
- [ ] **6397** - TODO(captainurist): target_pid is ignored here - the arrow re-resolves its target in castSpell() with a different fallback, so it can fly at a different actor than the one checked above.
...
- [ ] **281** - TODO(Nik-RE-dev): use getCharacterIdInParty directly where this function is called.
...
- [ ] **208** - TODO(captainurist): this is a weird place to call postGenerate, and we won't call it for items that end up buried. But it's here b/c we don't want to break the traces, at least now. Redo this properly and write a small test.
...
### `src/Engine/Objects/Decoration.h`

- [ ] **28** - TODO(captainurist): should be passed to eventProcessor explicitly, not smuggled in through a global.
  - Added since the last refresh. The decoration id reaches the event processor through a global instead of a parameter, so threading it through eventProcessor is the fix - touches the call sites that currently rely on the global.
...
- [ ] **739** - TODO(captainurist): change to 2d4+2.
...
- [ ] **429** - TODO(captainurist): do a honest parse of air/ground.
...
- [ ] **74** - TODO(captainurist): MonsterId for MONSTER_SPECIAL_ABILITY_EXPLODE, DamageType for MONSTER_SPECIAL_ABILITY_EXPLODE. Split.
...
- [ ] **53** - TODO(pskelton): refactor this so check isnt needed
...
- [ ] **216** - TODO(pskelton): move to collisions
...
- [ ] **365** - TODO(Nik-RE-dev): get rid of goto here
...
- [ ] **366** - TODO(pskelton): move to Collisions
...
- [ ] **7** - TODO(captainurist): drop this header
...
- [ ] **303** - #time replace with GetCurrentCivilTime().
...
- [ ] **387** - make 0-based with -1 for none??
...
- [ ] **482** - Do this properly for every single enum in this file.
...
- [ ] **116** - TODO(pskelton): caster index not supplied to buffs ".Apply"
...
- [ ] **2785** - TODO(captainurist): reimplement this in a saner way.
...
- [ ] **3200** - TODO(Nik-RE-dev): need to get rid of uPlayerID_2 and use pid with OBJECT_Character and something else for hirelings.
...
- [ ] **174** - TODO(captainurist): #unicode this is not ascii
...
- [ ] **181** - TODO(captainurist): #unicode this is not ascii
...
- [ ] **21** - TODO(Nik-RE-dev): It seems that two greet flags are used purely because it's modification is performed before greeting string is constructed. It is also ensures that NPC in multi-NPC houses always greet you with first line until you leave the house. Ideally there should be only one flag.
...
- [ ] **21** - TODO(captainurist): isTurnBased is just a synonym for 'really-totally-paused'. Doesn't belong here, move out. _(was `src/Engine/Time/Timer.h:21`)_
...
- [ ] **202** - TODO(pskelton): this function may modify frameRect - extract out
...
- [ ] **1026** - TODO(captainurist): Unload() un-pauses the game timer, which is not always the right thing to do. So we hack. Find a better way.
...
- [ ] **195** - TODO(Nik-RE-dev): need separate function for teleportation to other maps
...
- [ ] **161** - TODO(Nik-RE-dev): need separate function for teleportation to other maps
...
- [ ] **277** - TODO(pskelton): This doesnt work properly and we dont want draw calls here
...
- [ ] **1116** - TODO(pskelton): this looks duplicated extract
...
- [ ] **1371** - TODO(pskelton): could stretch texture rather than rescale
...
- [ ] **440** - TODO(Nik-RE-dev): untested until houses NPC can join the party
...
- [ ] **521** - TODO(Nik-RE-dev): can use GUIWindow_Transition
...
- [ ] **83** - TODO(captainurist): lazy-load.
...
- [x] **25** - find a better place for this code
...
- [x] **12** - #include "Io/InputEnumFunctions.h" // doesn't belong here
...
- [ ] **10** - just use raw file io, not FILE*
...
- [ ] **527** - it kinda sucks we can't just do toDebugString for enums via magic enum in Utility. Fix this?
...
- [ ] **575** - TODO(captainurist): this is temporary, we need separate axis enum and proper axis handling
...
- [ ] **164** - TODO: on android without this it won't close application properly until it finishes music track?! Something is not closing and preventing proper teardown?
...
- [ ] **508** - TODO(captainurist): uncomment this
...
- [ ] **911** - TODO(pskelton): check pointer maths for updating fly and waterwalk overlays
...
- [ ] **912** - TODO(pskelton): Split function up
...
- [ ] **565** - TODO(pskelton): stencil masking with opacity would be a better way to do this
...
- [ ] **247** - TODO(captainurist): projectiles can fly right through small actors - an ice bolt aimed at a peasant ~250 units away missed it and hit a titan further along the flight path.
...
- [ ] **19** - TODO(captainurist): This is needed because we roll back time in tests. We're dancing around with 32_ticks to maintain trace compatibility. Think how to do this better. _(was `src/Engine/Time/Timer.cpp:19`)_
...
- [ ] **204** - TODO(pskelton): Derived Messagebox types for different kinds of popup boxes
...
- [ ] **586** - TODO(captainurist): extend this to all encodings once ztd.text is fixed - decoding a single (odd) byte in the fixed-width UTF-16 / UTF-32 encodings reads out of bounds, and an unmapped byte in some multi-byte encodings may still hang.
...
## Design (278 open of 285)

_Blocked on a decision, missing data, or original-game research._

### `src/Application/Game.cpp`

- [ ] **592** - TODO(Nik-RE-dev): what is this? Btw, 153 == HOUSE_EARTH_GUILD_STONE_CITY.
...
- [ ] **747** - TODO(Nik-RE-dev): rest and heal uncoditionally even if party does not have food?
...
- [ ] **835** - TODO(captainurist): what's going on here?
...
- [ ] **79** - TODO(Gerark) Remove this GUI_UpdateWindows once we have a proper Retained Mode UI system. Right now we're forced to call this to cause the proper removal of temporary "buttons"
...
- [ ] **544** - TODO(captainurist): That's a very convoluted way to exit the game, redo this properly once we have a unified event loop.
...
- [ ] **1524** - TODO(captainurist): this was drawn with blendMode = 1 in original binary, did it have special meaning?
...
### `src/Core/Serialization/PlatformKeySerialization.cpp`

- [ ] **7** - these should be localizable? _(was `src/Io/InputEnumFunctions.cpp:173`)_
...
- [ ] **80** - TODO(captainurist): #time This is something to look at, we have comparisons with GameTime() in the code, they are not the same as Valid(). _(was `src/Engine/Time/Time.h:80`)_
...
### `src/Core/Trace/EventTrace.cpp`

- [ ] **234** - well, nlohmann json is retarded in that it chokes if we throw exceptions inside to_json calls for individual elements. Fix upstream? Note: there is an example in tests to reproduce. _(was `src/Library/Trace/EventTrace.cpp:222`)_
...
- [ ] **14** - this should go to Core/, not Library/, _(was `src/Library/Trace/EventTrace.h:14`)_
...
### `src/Engine/Data/SoundEnums.h`

- [ ] **227** - played back at speech volume? _(was `src/Media/Audio/SoundEnums.h:225`)_
...
- [ ] **340** - TODO: either add conversion functions, or keep only glm / only Vec3_* classes.
...
- [ ] **547** - TODO(captainurist): need to zero this one out when loading a save, but is this a proper place to do that?
...
- [ ] **729** - TODO(captainurist): try resurrecting the food / gold animations using resource files from MM6?
...
- [ ] **1054** - TODO(pskelton): adding this here for now but behaviour around firespike permanence needs checking
...
- [ ] **1058** - TODO(pskelton): do water and lava damage need to be more accurate to dt?
...
- [ ] **1069** - TODO(pskelton): fire damage?
...
- [ ] **1090** - TODO(captainurist): #time drop once we move to msecs in duration.
...
- [ ] **1217** - TODO: actually this looks like it never triggers. we get cursed_times, which is a time the character was cursed since the start of the game (a very large number), and compare it with times_triggered, which is a small number
...
- [ ] **1234** - TODO: cursed_times was a pointer before, and we had cursed_times = 0 here, was this meant to cancel the curse?
...
- [ ] **903** - TODO(yoctozepto): which are present in global events and which in local?
...
- [ ] **904** - TODO(yoctozepto): not present in used MM7 data
...
- [ ] **937** - TODO(captainurist): Is this correct? Houses can have ids > 255.
...
- [ ] **956** - TODO(yoctozepto): some types (marked with further TODOs) are not present in used MM7 data - their parsing might thus be wrong
...
- [ ] **968** - TODO(yoctozepto): always 1 in MM7 data, check MM6&8
...
- [ ] **1010** - TODO(yoctozepto): why add 1? it is not done with Event_CheckSkill
...
- [ ] **1029** - TODO(yoctozepto): always 0 in MM7 data, check MM6&8
...
- [ ] **1051** - TODO(yoctozepto): not present in used MM7 data
...
- [ ] **1073** - TODO(yoctozepto): not present in used MM7 data
...
- [ ] **1080** - TODO(yoctozepto): not present in used MM7 data; likely present in MM6
...
- [ ] **1083** - TODO(yoctozepto): not present in used MM7 data
...
- [ ] **1103** - TODO(yoctozepto): not present in used MM7 data; likely present in MM8 (e.g., Escaton's riddles)
...
- [ ] **1141** - TODO(yoctozepto): always 0 in MM7 data, check MM6&8
...
- [ ] **1152** - TODO(yoctozepto): not present in used MM7 data
...
- [ ] **1153** - TODO(yoctozepto): not present in used MM7 data
...
- [ ] **1174** - TODO(yoctozepto): always 0 in MM7 data, check MM6&8
...
- [ ] **1183** - TODO(yoctozepto): always 0 in MM7 data, check MM6&8
...
- [ ] **1189** - TODO(yoctozepto): always 0 in MM7 data, check MM6&8
...
- [ ] **1213** - TODO(yoctozepto): not present in used MM7 data
...
- [ ] **1216** - TODO(yoctozepto): not present in used MM7 data
...
- [ ] **1219** - TODO(yoctozepto): not present in used MM7 data
...
- [ ] **1222** - TODO(yoctozepto): not present in used MM7 data
...
- [ ] **1225** - TODO(yoctozepto): not present in used MM7 data
...
- [ ] **1228** - TODO(yoctozepto): not present in used MM7 data
...
- [ ] **1231** - TODO(yoctozepto): not present in used MM7 data
...
- [ ] **1234** - TODO(yoctozepto): not present in used MM7 data
...
- [ ] **172** - TODO: enconunter and process
...
- [ ] **212** - TODO(pskelton): Fix #1890 this should be a data mod
...
- [ ] **223** - TODO(pskelton): Fix #2117 this should be a data mod
...
- [ ] **366** - TODO(pskeltonm): Fix #2223 stop tutorial message spam - should be data mod
...
- [ ] **391** - TODO(Nik-RE-dev): this event is not used in MM7. In GrayFace's data it's called "Question" and must have additional arguments that control where events executions must be continued on correct/incorrect input.
...
- [ ] **468** - TODO(Nik-RE-dev): Looks like it's artifact of MM6
...
- [ ] **504** - TODO(captainurist): make the state<->event mapping per-game before feeding MM8 data through here
  - Added by #2615. MM6/MM7 store id-124 in a byte and dispatch at +380, MM8 is piecewise (states 0-21 to events 268-289, 23-62 to 531-570). Needs the per-game mapping before MM8 data can flow through, so it is blocked on MM8 support rather than on a local fix.
- [ ] **514** - TODO(Nik-RE-dev): original code for this option is dubious
...
- [ ] **525** - TODO: enconunter and process
...
- [ ] **551** - TODO: enconunter and process
...
- [ ] **563** - TODO: enconunter and process
...
- [ ] **29** - TODO(pskelton): do we want to be overriding the config value here?
...
- [ ] **87** - TODO(captainurist): up is negative? wtf???
...
- [ ] **969** - TODO(pskelton): Better way to do this? Maybe add a climbable attribute
...
- [ ] **991** - TODO(pskelton): This 'catch all' is probably unsafe - would be better as above
...
- [ ] **63** - TODO(captainurist): move to SoundEnums.h?
...
- [ ] **784** - TODO(pskelton): asserts trips on test 416 with Dragons OOB - consider running actor check on file load to correct positions so this assert can be reinstated.
...
- [ ] **843** - TODO(pskelton): why 8? doesnt match party
...
- [ ] **1052** - TODO: Does POLYGON_Ceiling really belong here? Returned z is then used like this in UpdateActors_BLV: if (actor.z <= z) { actor.z = z + 1; And if this z is ceiling z, then this will place the actor above the ceiling.
...
- [ ] **1504** - TODO(captainurist): #time think about a better way to write this formula.
...
- [ ] **1671** - TODO(Nik-RE-dev): need to probe surface
...
- [ ] **1682** - TODO(Nik-RE-dev): need to probe surface
...
- [ ] **1868** - TODO(captainurist): this needs checking
...
- [ ] **86** - TODO(captainurist): why is this one unused?
...
- [ ] **181** - TODO(captainurist): why is this one unused?
...
- [ ] **771** - TODO(pskelton): drop tint color anyway?
...
- [ ] **995** - TODO(captainurist): #time think about a better way to write this formula.
...
- [ ] **1681** - TODO(pskelton): this cancels out the above - is this intended
...
- [ ] **11** - TODO(pskelton): Overlays in MM7/ MM8 are blank so most of this isnt used. MM6 does have overlays - investigate if needed
...
- [ ] **124** - TODO(captainurist): checking animTimer->isPaused(), then using gameTimer->uTimeElapsed?
...
- [ ] **203** - OpenEnroth do not support mods and data patches right now. TODO(pskelton): data patch So the changes are in C++ code. But it better be placed in lua scripts or binary data patches.
...
- [ ] **641** - TODO(yoctozepto, pskelton): we should probably try to handle these faces as they are otherwise marked as visible (see also BSPRenderer)
...
- [ ] **765** - TODO(pskelton): should this call drawworld instead??
...
- [ ] **2250** - TODO(pskelton): might have to pass a texture width through for the waterr flow textures to size right and get the correct water speed
...
- [ ] **2291** - TODO(yoctozepto, pskelton): we should probably try to handle these faces as they are otherwise marked as visible (see also BSPRenderer)
...
- [ ] **2383** - TODO(pskelton): any instances where animTime is not consistent would need checking
...
- [ ] **2750** - TODO(yoctozepto, pskelton): we should probably try to handle these faces as they are otherwise marked as visible (see also BSPRenderer)
...
- [ ] **3248** - TODO(yoctozepto, pskelton): we should probably try to handle these faces as they are otherwise marked as visible (see also BSPRenderer)
...
- [ ] **187** - TODO(pskelton): investigate and fix properly - dragon breath is missing last two frames??
...
- [ ] **167** - TODO: WTF? 184 / 185 qbits are associated with Tatalia's Mercenery Guild Harmondale raids. Are these about castle's tapestries ?
...
- [ ] **59** - TODO(captainurist): should be moved to localization files eventually
...
- [ ] **238** - TODO(captainurist): why not LSTR_ACOLYTE?
...
- [ ] **319** - TODO(captainurist): Not currently used anywhere
...
- [ ] **584** - TODO(Nik-RE-dev): is check for boots correct?
...
- [ ] **1063** - TODO(captainurist): these are some weird casts to CharacterAttributeType
...
- [ ] **1067** - TODO(_): determine if spirit resistance should be handled by body res. modifier
...
- [ ] **1200** - TODO(captainurist): returns not used - should luck attribute affect?
...
- [ ] **1393** - TODO(captainurist): can't break wands b/c they are not regular items. Makes little in-game sense IMO.
...
- [ ] **1408** - TODO(captainurist): This can break a wetsuit, and this looks like vanilla behavior. But the code in SPECIAL_ATTACK_BREAK_ANY can't break a wetsuit. Huh.
...
- [ ] **1423** - TODO(captainurist): why doesn't this affect wands?
...
- [ ] **2560** - TODO(_): move the individual implementations to attribute classes once possible ?? check
...
- [ ] **3086** - TODO(Nik-RE-dev): no CanAct check?
...
- [ ] **5704** - TODO(pskelton): check - should this be able to forget a skill '0' or min of '1'
...
- [ ] **5705** - TODO(pskelton): check - should this modify mastery as well
...
- [ ] **5966** - TODO(captainurist): should this include other projectile attacks? Vanilla only checks arrows.
...
- [ ] **5967** - TODO(captainurist): should we check that the character is actually unarmed? Vanilla doesn't check, apparently.
...
- [ ] **6092** - TODO(captainurist): another weird default.
...
- [ ] **6758** - TODO(pskelton): firespike meant to remain permanantly??
...
- [ ] **6779** - TODO(captainurist): isn't it weird that promotions aren't included in comparisons here?
...
- [ ] **292** - TODO(captainurist): check all usages, most should be using getActualSkillValue.
...
- [ ] **132** - TODO(captainurist): TBH this makes no sense, investigate
...
- [ ] **139** - TODO(captainurist): TBH this makes no sense, investigate
...
- [ ] **509** - TODO(Nik-RE-dev): is this correct?
...
- [ ] **635** - TODO(captainurist): item->uSoundID & 8 checks for laser projectiles, wtf...
...
- [ ] **243** - check if this condition really should be here. it is equal to the original source but still seems kind of weird
...
- [ ] **669** - We overwrite the random timing from the PORTRAIT_NORMAL branch here. Doesn't seem intentional!
...
- [ ] **124** - disable flashing for all books until we save state to savegame file
...
- [ ] **597** - vanilla MM7 only allocated memory for 500 actors, 1000 sprite objects, and 20 chests at runtime. We should either cap these or fail gracefully on save to maintain compatibility.
...
- [ ] **172** - do we need to check for overflows here?
...
- [ ] **185** - do we need to check for overflows here?
...
- [ ] **198** - do we need to check for overflows here?
...
- [ ] **211** - do we need to check for overflows here?
...
- [ ] **230** - do we need to check for overflows here?
...
- [ ] **348** - Can drop? Need to check that vanilla doesn't choke here.
...
- [ ] **230** - TODO(Nik-RE-dev): does scrolls must fail?
...
- [ ] **277** - TODO(pskelton): was pParty->uPartyHeight / 2
...
- [ ] **505** - TODO(pskelton): was pParty->uPartyHeight / 2
...
- [ ] **2024** - TODO: is this correct? Spell description says that spell effect is infinite.
...
- [ ] **2693** - TODO(pskelton): was pParty->uPartyHeight / 2
...
- [ ] **3138** - TODO: if no more place for spells in queue then spell is just ignored? Need assert?
...
- [ ] **3202** - TODO(Nik-RE-dev): why recovery time is set here?
...
- [ ] **283** - TODO(captainurist): Caching in kinda wrong? Revisit when working on #mm6. See "O Ho! %13! Er, %13. I think. Whatever..."
...
- [ ] **438** - TODO(pskelton): check tickcount usage here
...
- [ ] **806** - TODO(captainurist): ^ and what about night?
...
- [ ] **1081** - TODO(_) fix these and move them up before the window check loop.
...
- [ ] **370** - TODO(pskelton): check this behaviour
...
- [ ] **707** - TODO(captainurist): this is a weird access into pTransitionStrings, investigate & add docs
...
- [ ] **813** - TODO(Nik-RE-dev): maybe need to unify selectColor for all dialogue
...
- [ ] **141** - TODO(Nik-RE-dev): is this correct?
...
- [ ] **149** - TODO(Nik-RE-dev): if message is special then no video when entering indoor?
...
- [ ] **50** - SDL orientation code turned out to be buggy and works only before window creation, hardcode only landscape modes there for now.
...
- [ ] **194** - do we need to reinstate this optimisation? dropped to allow better sound tracing
...
- [ ] **273** - Vanilla sounds like it does unique id but as exclusives
...
- [ ] **296** - TODO(pskelton): fixing cylinder collisions means we can no longer get into this hole
...
### `test/Bin/RetraceTest/RetraceTest.cpp`

- [ ] **28** - TODO(captainurist): #cpp26 use std::sat_sub _(was `src/Bin/OpenEnroth/OpenEnroth.cpp:48`)_
...
- [ ] **109** - TODO(pskelton): Tape will be spammy as it is recording everything.
...
- [ ] **2748** - TODO(pskelton): ?? needs changing - check behavious
