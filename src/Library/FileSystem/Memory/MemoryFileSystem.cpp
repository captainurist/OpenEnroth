#include "MemoryFileSystem.h"

#include <memory>
#include <string>
#include <utility>
#include <vector>

#include "Utility/Error/Result.h"
#include "Library/FileSystem/Interface/FileSystemError.h"
#include "Utility/String/Join.h"
#include "MemoryFileSystemInputStream.h"
#include "MemoryFileSystemOutputStream.h"

MemoryFileSystem::MemoryFileSystem(std::string_view displayName) : _displayName(displayName) {}

void MemoryFileSystem::clear() {
    _trie.clear();
}

Result<bool> MemoryFileSystem::_exists(FileSystemPathView path) const {
    assert(!path.isEmpty());
    return _trie.find(path) != nullptr;
}

Result<FileStat> MemoryFileSystem::_stat(FileSystemPathView path) const {
    assert(!path.isEmpty());

    const Node *node = _trie.find(path);
    if (!node)
        return FileStat();

    if (node->hasValue()) {
        return FileStat(FILE_REGULAR, node->value()->blob.size());
    } else {
        return FileStat(FILE_DIRECTORY, 0);
    }
}

Result<void> MemoryFileSystem::_ls(FileSystemPathView path, std::vector<DirectoryEntry> *entries) const {
    const Node *node = _trie.find(path);
    if (!node)
        return fileSystemError(this, FS_LS_FAILED_PATH_DOESNT_EXIST, path);
    if (node->hasValue())
        return fileSystemError(this, FS_LS_FAILED_PATH_IS_FILE, path);

    for (const auto &[name, child] : node->children())
        entries->push_back(DirectoryEntry(name, child->hasValue() ? FILE_REGULAR : FILE_DIRECTORY));
    return {};
}

Result<Blob> MemoryFileSystem::_read(FileSystemPathView path) const {
    // We mimic how Windows handles file mapping here - treating mapped files as if they are open for reading.
    const Node *node = co_await nodeForReading(path);
    std::shared_ptr<MemoryFileData> data = node->value();
    data->readerCount++;
    std::shared_ptr<void> guard(nullptr, [data](void *) { data->readerCount--; });
    co_return Blob::custom(data->blob.data(), data->blob.size(), std::move(guard)).withDisplayPath(displayPath(path));
}

Result<void> MemoryFileSystem::_write(FileSystemPathView path, const Blob &data) {
    (co_await nodeForWriting(path))->value()->blob = Blob::share(data).withDisplayPath(displayPath(path));
}

Result<std::unique_ptr<InputStream>> MemoryFileSystem::_openForReading(FileSystemPathView path) const {
    co_return std::unique_ptr<InputStream>(std::make_unique<detail::MemoryFileSystemInputStream>((co_await nodeForReading(path))->value()));
}

Result<std::unique_ptr<OutputStream>> MemoryFileSystem::_openForWriting(FileSystemPathView path) {
    co_return std::unique_ptr<OutputStream>(std::make_unique<detail::MemoryFileSystemOutputStream>((co_await nodeForWriting(path))->value(), displayPath(path)));
}

Result<void> MemoryFileSystem::_rename(FileSystemPathView srcPath, FileSystemPathView dstPath) {
    assert(!srcPath.isEmpty());
    assert(!dstPath.isEmpty());

    Node *srcNode = _trie.find(srcPath);
    if (!srcNode)
        return fileSystemError(this, FS_RENAME_FAILED_SRC_DOESNT_EXIST, srcPath, dstPath);

    FileSystemPathView dstTail;
    Node *dstNode = _trie.walk(dstPath, &dstTail);
    if (dstTail.isEmpty()) { // dstPath exists.
        if (!dstNode->hasValue())
            return fileSystemError(this, FS_RENAME_FAILED_DST_IS_DIR, srcPath, dstPath);
        if (!srcNode->hasValue())
            return fileSystemError(this, FS_RENAME_FAILED_SRC_IS_DIR_DST_IS_FILE, srcPath, dstPath);
    }

    _trie.insertOrAssign(dstNode, dstTail, _trie.extract(srcNode));
    return {};
}

Result<bool> MemoryFileSystem::_remove(FileSystemPathView path) {
    assert(!path.isEmpty());

    Node *node = _trie.find(path);
    if (!node)
        return false;

    return _trie.erase(node);
}

std::string MemoryFileSystem::_displayPath(FileSystemPathView path) const {
    return join(_displayName, "://", path.string());
}

Result<const MemoryFileSystem::Node *> MemoryFileSystem::nodeForReading(FileSystemPathView path) const {
    assert(!path.isEmpty());
    const Node *node = _trie.find(path);
    if (!node)
        return fileSystemError(this, FS_READ_FAILED_PATH_DOESNT_EXIST, path);
    if (!node->hasValue())
        return fileSystemError(this, FS_READ_FAILED_PATH_IS_DIR, path);
    if (node->value()->writerCount)
        return fileSystemError(this, FS_READ_FAILED_PATH_NOT_READABLE, path);
    return node;
}

Result<MemoryFileSystem::Node *> MemoryFileSystem::nodeForWriting(FileSystemPathView path) {
    assert(!path.isEmpty());

    FileSystemPathView tail;
    Node *node = _trie.walk(path, &tail);

    if (!tail.isEmpty()) { // File doesn't exist.
        if (node->hasValue()) {
            return fileSystemError(this, FS_WRITE_FAILED_FILE_IN_PATH, path);
        } else {
            return _trie.insertOrAssign(node, tail, std::make_shared<MemoryFileData>(Blob()));
        }
    }

    if (!node->hasValue())
        return fileSystemError(this, FS_WRITE_FAILED_PATH_IS_DIR, path);

    if (node->value()->readerCount || node->value()->writerCount)
        return fileSystemError(this, FS_WRITE_FAILED_PATH_NOT_WRITEABLE, path);

    return node;
}

