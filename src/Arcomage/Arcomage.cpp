...
bool ArcomageGame::LoadSprites() {
    ...
    for (Color &pixel : pArcomageGame->pSprites->rgba().pixels())
        if (pixel == colorTable.Blue)
            pixel = Color();
    render->Update_Texture(pArcomageGame->pSprites.get());
...
void ArcomageGame::Loop() {
    ...
    pArcomageGame->pGameBackground.reset();
    pArcomageGame->pSprites.reset();
...
void DrawGameUI(int animation_stage) {
    render->BeginScene2D();
    // draw background
    render->DrawTextureNew(0, 0, pArcomageGame->pGameBackground.get());
...
void DrawRectanglesForText() {
    ...
    pTargetXY.x = 8;
    pTargetXY.y = 56;
    render->DrawFromSpriteSheet(pArcomageGame->pSprites.get(), pSrcRect, pTargetXY, colorTable.White);
...
void DrawPlayerLevels(std::string_view str, Pointi *pXY) {
    ...
    for (char i : str) {
        ...
        if (i) {
            ...
            render->DrawFromSpriteSheet(pArcomageGame->pSprites.get(), pSrcRect, pTargetPoint, colorTable.White);
            pTargetPoint.x += 22;
        }
    }
}
...
void DrawPlayersTowers() {
    ...
    if (tower_height > 0) render->DrawFromSpriteSheet(pArcomageGame->pSprites.get(), pSrcXYZW, pTargetXY, colorTable.White);  //стена башни
    ...
    pTargetXY.x = 91;
    render->DrawFromSpriteSheet(pArcomageGame->pSprites.get(), pSrcXYZW, pTargetXY, colorTable.White);  //верхушка башни
    ...
    pTargetXY.x = 494;
    pTargetXY.y = 297 - tower_top;
    if (tower_height > 0) render->DrawFromSpriteSheet(pArcomageGame->pSprites.get(), pSrcXYZW, pTargetXY, colorTable.White);
    ...
    pTargetXY.x = 483;
    pTargetXY.y = 203 - tower_top;
    render->DrawFromSpriteSheet(pArcomageGame->pSprites.get(), pSrcXYZW, pTargetXY, colorTable.White);
}
    ...
    for (int card_slot = 0; card_slot < card_count; ++card_slot) {
        ...
        if (am_Players[current_player_num].cards_at_hand[card_slot] == -1) {
            ...
            if (am_Players[current_player_num].IsHisTurn == 0 && See_Opponents_Cards == 0) {
                ...
                render->DrawFromSpriteSheet(pArcomageGame->pSprites.get(), pSrcXYZW, pTargetXY, colorTable.White);  //рисуется оборотные стороны карт противника
            } else {
                ...
                if (!CanCardBePlayed(current_player_num, card_slot)) {
                    ...
                    render->DrawFromSpriteSheet(pArcomageGame->pSprites.get(), pSrcXYZW, pTargetXY, colorTable.Gray);
                } else {
                    render->DrawFromSpriteSheet(pArcomageGame->pSprites.get(), pSrcXYZW, pTargetXY, colorTable.White);  //рисуются активные карты
                }
            }
        }
    ...
    for (int table_cards = 0; table_cards < 10; ++table_cards) {
        if (hide_card_anim_runnning == 0) {
            ...
            if (shown_cards[table_cards].uCardId != -1) {
                ...
                render->DrawFromSpriteSheet(pArcomageGame->pSprites.get(), pSrcXYZW, shown_cards[table_cards].hide_anim_pos, colorTable.Gray);
            }
...
void DrawCardAnimation(int animation_stage) {
    ...
    if (drawn_card_slot_index != -1) {
        if (drawn_card_anim_cnt >= 9) {
            ...
            pSrcXYZW.w = 288 - pSrcXYZW.x;
            pSrcXYZW.h = 128 - pSrcXYZW.y;
            render->DrawFromSpriteSheet(pArcomageGame->pSprites.get(), pSrcXYZW, anim_card_pos_drawncard, colorTable.White);
        } else {
    ...
    if (discarded_card_id != -1) {
        ...
        if (animation_stage <= 10) {
            ...
            render->DrawFromSpriteSheet(pArcomageGame->pSprites.get(), pSrcXYZW, anim_card_pos_playdiscard, colorTable.Gray);
        }
    }
    ...
    if (played_card_id != -1) {
        ...
        if (animation_stage > 15.0) {
            ...
            anim_card_pos_playdiscard.x += anim_card_spd_playdiscard.x;
            anim_card_pos_playdiscard.y += anim_card_spd_playdiscard.y;
            render->DrawFromSpriteSheet(pArcomageGame->pSprites.get(), pSrcXYZW, anim_card_pos_playdiscard, colorTable.White);
            return;
        }
