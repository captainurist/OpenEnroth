...
void GUIProgressBar::Release() {
    loading_bg.reset();
    progressbar_loading.reset();
    progressbar_dungeon.reset();
    uType = TYPE_None;
}
...
void GUIProgressBar::Draw() {
    ...
    if (uType != TYPE_Fullscreen) {
        ...
        render->DrawTextureNew(80 / 640.0f, 122 / 480.0f, progressbar_dungeon.get());
        render->DrawTextureNew(100 / 640.0f, 146 / 480.0f, pIconsFrameTable->animationFrame(turnHourIconId, 0_ticks).get());
        render->FillRectFast(174, 164, floorf(((double)(113 * uProgressCurrent) / (double)uProgressMax) + 0.5f), 16, colorTable.Red);
    } else {
        if (loading_bg) {
            render->DrawTextureNew(0, 0, loading_bg.get());
        }
        render->SetUIClipRect(Recti(172, 459, (int)((double)(300 * uProgressCurrent) / (double)uProgressMax), 12));
        render->DrawTextureNew(172 / 640.0f, 459 / 480.0f, progressbar_loading.get());
