...
void GUIWindow_BranchlessDialogue::Update() {
    ...
    BranchlessDlg_window.frameRect.w = game_viewport_width;
    int pTextHeight = assets->pFontArrus->CalcTextHeight(branchless_dialogue_str, BranchlessDlg_window.frameRect.w, 12) + 7;
    if (352 - pTextHeight < 8) {
        pFont = assets->pFontCreate.get();
        pTextHeight = assets->pFontCreate->CalcTextHeight(branchless_dialogue_str, BranchlessDlg_window.frameRect.w, 12) + 7;
    }
    ...
    pGUIWindow_BranchlessDialogue->DrawText(pFont, {12, 354 - pTextHeight}, colorTable.White,
                                            pFont->FitTextInAWindow(branchless_dialogue_str, BranchlessDlg_window.frameRect.w, 12));
