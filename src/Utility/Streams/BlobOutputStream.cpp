...
Result<void> BlobOutputStream::_overflow(Buffer *buffer, const void *data, size_t size) {
...
Result<void> BlobOutputStream::_flush(Buffer *buffer) {
...
Result<void> BlobOutputStream::_close(Buffer *buffer) {
    assert(isOpen());

    *_target = _scratchpad.finish(position()).withDisplayPath(displayPath());
    _target = nullptr;

    return base_type::_close(buffer);
}
