...
Result<Blob> Blob::fromFile(std::string_view path) {
    ...
    // On Windows mio::mmap_source expects UTF8-encoded paths. If the file doesn't exist, std::system_error is
    // thrown - mio is external code, so we catch here, per our error handling rules.
    ...
    try {
        ...
        return Error(e.code(), "{}: {}", pathString, e.code().message());
    }
