...
* The (de)serialization layer is written in the coroutine style: struct-level deserializers are coroutines that chain their reads with `co_await`, which acts exactly like `MM_TRY` but as a language construct. Leaf reads and per-element loops are plain functions – never write a per-element or per-byte coroutine. See `ExceptionFreeErrorHandling.md` §3 and the coroutine docs in `Result.h` (including the `co_return {};` footgun).
